...
def LoadArticles(dirname,fname):
    ...
    if os.path.isfile(dirname+fname):
        ...
        Title=Abstract=Body=RefTo=Authors=RefBy=DatePub=url=LibName=PMID=PMCID=DOI=''
        df = pd.DataFrame([[Title,Abstract,Body,RefTo,Authors,RefBy,DatePub,url,LibName,PMID,PMCID]],
            columns = ['Title','Abstract','Body','RefTo','Authors','RefBy','DatePub','url','LibName','PMID','PMCID'],
            index=[DOI]
            )
        df= df.astype({'Title':'string','Abstract':'string','Body':'string','DatePub':'string','url':'string','LibName':'string','PMID':'string','PMCID':'string'})
...
def AddArticles(DOI,df=None,Title='',Abstract='',Body='',RefTo='',Authors='',
    RefBy='',DatePub='',url='',LibName='',PMID='',PMCID=''):
    ...
    if isinstance(df,type(None)):
        ...
        df.loc[DOI]= [Title,Abstract,Body,RefTo,Authors,RefBy,DatePub,url,LibName,PMID,PMCID]
    return(df)
